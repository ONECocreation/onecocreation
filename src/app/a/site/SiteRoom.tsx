 ...
 * link — each rail shows only its own knobs).
 *
 * TASK-188 (0018.06.18 a₿ · block 966,112): "it's very much a lot right
 * now" (the Admiral) — the Site room breathes. Switches keeps /a/site (the
 * default sub-room); the Community door, the Menu editor, and the Videos on
 * About playlist each moved to their own sub-route (/a/site/community-door,
 * /a/site/menu, /a/site/about-videos) with an accordion under "Site" in the
 * left rail. The cards themselves are untouched — same self-contained
 * fetch/save, same Save for the switches alone.
 */
...
const MEETING_RAILS: { key: SiteConfig["meeting"]["rail"]; label: string; about: string }[] = [
  ...
  { key: "vdo", label: "The studio", about: "sessions meet in your studio — guest and director links on this site" },
  ...
  return (
    <div className="p-6" style={{ maxWidth: 860 }}>
      ...
      {config.meeting.rail === "vdo" && (
        <div style={row}>
          <div style={{ flex: 1, minWidth: 220 }}>
            <b style={{ fontSize: ".9rem" }}>Studio room prefix</b>
