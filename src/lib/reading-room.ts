...
/** TASK-465 (block 968,561) — MOVED from `clair-senses` to
 *  `weekly-reading`. The Admiral: "weekly intuitive room for clair senses
 *  room can be hidden for now. we are only going to have the playground in
 *  that room area" — reversing TASK-460's pick (block 968,543, decision
 *  cw-playground-where, option B, which put it on `clair-senses`). Named
 *  once so it never rides as a bare string twice — the ONE room
 *  StageView.tsx opens a "Join the Playground call" door in
 *  (`matrix-rooms.ts`'s own `weekly-reading` alias; the room's TITLE is
 *  "The Playground" there now). */
export const PLAYGROUND_ROOM_SLUG = "weekly-reading";
