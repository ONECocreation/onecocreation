 ...
 *
 * TASK-279 (0018.06.25 a₿): moved here from fren-auth.ts — same
 * implementation, the four Fren-named exports renamed to their Member
 * equivalents. The cookie NAME/VALUE (`MEMBER_COOKIE = "pa-fren"`) is
 * UNCHANGED — only the export identifier moved; the old names are
 * re-exported below as aliases (same function objects) so fren-auth.ts's
 * compat shim, and any importer still on it, keep working unchanged.
 */
...
export function makeMemberToken(handle: string, space: string): string {
...
export function memberFromRequest(request: Request): { handle: string; space: string } | null {
  const sessions = sessionsFromRequest(request);
  return sessions.length ? { handle: sessions[0].handle, space: sessions[0].space } : null;
}

/* TASK-279 compat aliases — the OLD identifiers, re-exported as the SAME
   function objects (not copies) so fren-auth.ts's compat shim and any
   not-yet-migrated importer keep working unchanged. */
export {
  memberFromRequest as frenFromRequest,
  makeMemberToken as makeFrenToken,
  verifyMemberLogin as verifyFrenLogin,
  MEMBER_COOKIE as FREN_COOKIE,
};
