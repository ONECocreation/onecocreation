 ...
 * `src/lib/member-auth.ts`, additive — but `member-auth.ts` carries a
 ...
 * auth.ts → registry.ts"). Importing ANYTHING from member-auth.ts into the
 * middleware — Web-Crypto-only functions included — drags registry.ts
 * along and fails the build. This file has NO import from member-auth.ts,
 ...
 * verify decision agree with member-auth.ts's own node path, token for
 * token, so the two can never quietly drift apart.
 *
 * TASK-279 (0018.06.25 a₿): moved here from fren-auth-edge.ts — same
 * implementation, `FREN_COOKIE` renamed to `MEMBER_COOKIE` (value stays
 * `"pa-fren"`, unchanged). `verifySessionTokenEdge`/`hasValidSessionEdge`
 * carried no "Fren" in their names, so they're unchanged. The old
 * `FREN_COOKIE` name is re-exported below as an alias (the SAME constant)
 * so fren-auth-edge.ts's compat shim, and tests/fren-auth-edge.test.ts,
 * keep working unchanged. Still zero import from ./member-auth or
 * ./registry — that constraint didn't move, it just moved WITH the file.
 */

/** Mirrors member-auth.ts's `MEMBER_COOKIE` — cannot import it (see the
 *  file docblock); pinned equal by tests/fren-auth-edge.test.ts. */
export const MEMBER_COOKIE = "pa-fren";
/** Mirrors member-auth.ts's private `TOKEN_JOIN` — same reason. */
const TOKEN_JOIN = "~";
...
/** One token, verified the Web-Crypto way — member-auth.ts's private
...
export async function hasValidSessionEdge(cookieHeader: string | null): Promise<boolean> {
  ...
  const match = cookie.match(new RegExp(`${MEMBER_COOKIE}=([^;]+)`));
...
/* TASK-279 compat alias — the OLD identifier, re-exported as the SAME
   constant so fren-auth-edge.ts's compat shim and tests/fren-auth-edge.
   test.ts keep working unchanged. */
export { MEMBER_COOKIE as FREN_COOKIE };
