    ...
    >
      ...
      <span className="block rounded-md border border-edge/80 bg-panel/95 px-3 py-2.5 backdrop-blur-sm">
        {/* the face — TIME the hero: big flip cards (hh:m + the struggling
            ones digit wearing the honest ~), the yyyy.mm.dd a₿ date tiny
            and dim below (clock hierarchy ruling + the canonical order) */}
        <span className={`flex justify-center font-mono text-[28px] ${estimated ? "opacity-70" : ""}`}>
          {/* the honest ~ lives ON the face when the height is a network-dark
              estimate (house law: tooltips don't count) */}
          {estimated && <span className="self-center pr-1 text-cyan/70">~</span>}
          <FlipClock height={height} fill={fill} />
        </span>

        {/* the door's handle */}
        <span className="mt-1 block text-center font-mono text-[6px] uppercase tracking-[0.3em] text-cyan/60">
          open the clock </span>
      </span>
    </Link>
  );
