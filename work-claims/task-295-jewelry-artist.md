# work-claim — task-295-jewelry-artist (T-295 wave A, pair 3: /jewelry + /artist become designer pages)

Lane: Ms. Kimi (sub-agent). Base = onecocreation main @ **0ea1be2** (the T-295 pair 1 merge, per K34).
Branch `feat/task-295-jewelry-artist`. Worktree cut by Number One; `node_modules` absent at
claim time → `npm ci` is the first act after this claim lands. Lane ports **4366–4369**.
...
- `src/lib/page-states.ts`: flip the `/jewelry` and `/artist` rows words → designer as they
...
3. Shots via `bash scripts/shots-fixture.sh --ports 4366-4369 --routes "/jewelry,/artist"
...
## OWNS
`src/app/jewelry/page.tsx`, `src/app/artist/page.tsx`, their seed regions in
`src/lib/puck-seeds.ts`, their rows in `src/lib/page-states.ts`, one test file per page,
`work-claims/task-295-jewelry-artist.md`. NOT `puck-config.tsx`/`copilot.ts`
(flag-and-stop), NOT `src/components/**`, NOT home, NOT wave B pages, NOT the other
pair's pages (/contact, /services).
...
## Report
`~/dev/kimi/outbox/task-295/jewelry-artist/SUMMARY.md` written AS YOU GO, ending once with
`LANE-DONE <full sha>`.
