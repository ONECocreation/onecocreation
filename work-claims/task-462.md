...
**Round 4 widening (block 968,548, Number One's own instruction):** round 3 reverted `entitlement-fulfil.ts` to base byte-for-byte and it is NOT currently claimed in OWNS above (the round-1 line describing it is stale history — that mechanism no longer exists). OWNS now covers `src/lib/entitlement-fulfil.ts` again, scoped to exactly ONE thing: the refunded/disputed branch's FIRST LINE — a new `if (await isLapsedPassOrder(npub, order.id)) return {...};` guard, added before `held = await getEntitlement(npub)`. Nothing else in the file changes: the room removal, `revokeTier(npub)`, and the closing letter all stay byte-identical to base for every case that doesn't hit this new guard (a still-live pass, the standing grant's own order, any settled/fulfilled path). `entitlement.ts` also gains one new pure function, `isLapsedPassOrder`, already fully in OWNS.

Read-only (never touched): matrix.ts, the webhook and checkout routes, the orders route, /a/store, room-access.ts, kit.css, member-tier.ts, src/app/api/admin/matrix/ceremony/route.ts, every other existing test file (existing pins run untouched), everything in `live.ts` outside `classStartingAudience`, and everything in `entitlement-fulfil.ts` outside the refunded/disputed branch's new first line.
