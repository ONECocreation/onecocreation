...
- **The returning-member state lost its floating Heart Field button** (Number One, after SEEING the full-page shots). It sat loose between the reused card and the agenda, and the stage card above already carries that door. This also removes the box's one inline style. The door right after a fresh sign-in stays; that is the moment the Admiral asked for it ("after email key login, give the option to open the watch now").
