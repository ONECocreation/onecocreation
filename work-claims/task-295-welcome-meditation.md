# work-claim — task-295-welcome-meditation (T-295 wave A, pair 1: /welcome + /meditation become designer pages)

Lane: Ms. Kimi (sub-agent). Base = onecocreation main @ **57bba3a** (the T-293 merge, per K31/K32).
Branch `feat/task-295-welcome-meditation`. Worktree cut by Number One; `node_modules` absent at
claim time → `npm ci` is the first act after this claim lands. Lane ports **4322–4325**.
...
- `src/lib/page-states.ts`: flip `/welcome` and `/meditation` rows words → designer as they land
  (`/packages` row is stale from T-232 — fix it too, per the GO's own note).
- Pages-panel manifest test `tests/pages-panel-states.test.ts` walks `src/app/*/page.tsx`.
- Words law: transcribe, never rewrite. The meditation line is "on the house, from our 💞 to
  yours" (T-290).
...
3. Shots via `bash scripts/shots-fixture.sh --ports 4322-4325 --routes "/welcome,/meditation"
...
## OWNS
`src/app/welcome/page.tsx`, `src/app/meditation/page.tsx`, their seed regions in
`src/lib/puck-seeds.ts`, their rows in `src/lib/page-states.ts`, one test file per page,
`work-claims/task-295-welcome-meditation.md`. NOT `puck-config.tsx`/`copilot.ts`
(flag-and-stop), NOT `src/components/**`, NOT home (T-293), NOT wave B pages, NOT the other
pair's pages.
...
## Report
`~/dev/kimi/outbox/task-295/welcome-meditation/SUMMARY.md` written AS YOU GO, ending once with
`LANE-DONE <full sha>`.
