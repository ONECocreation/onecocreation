# work-claim — task-295-contact-services (T-295 wave A, pair 4: /contact + /services become designer pages)

Lane: Ms. Kimi (sub-agent). Base = onecocreation main @ **0ea1be2** (the T-295 pair 1 merge, per K34).
Branch `feat/task-295-contact-services`. Worktree cut by Number One; `node_modules` absent at
claim time → `npm ci` is the first act after this claim lands. Lane ports **4370–4373**.
GO: `~/dev/kimi/inbox/TASK-295-oc-every-page-in-style-wave-a.md` + K34.
...
- `src/lib/page-states.ts`: flip the `/contact` and `/services` rows words → designer as they
...
3. Shots via `bash scripts/shots-fixture.sh --ports 4370-4373 --routes "/contact,/services"
   --cookie none` fallback AND published. The harness has NO seed flag upstream; the
   generalized `--seed-puck` variant lives at
   `~/dev/kimi/outbox/task-295/privacy-terms/shots-fixture-seeded.sh` (pair 2's copy) until
   T-294 lands it in `scripts/` — check `grep -n seed scripts/shots-fixture.sh` first and use
   the in-repo one if T-294 has landed. Both themes × 1440/390. Fallback = published, read and
   said.

## OWNS
`src/app/contact/page.tsx`, `src/app/services/page.tsx`, their seed regions in
`src/lib/puck-seeds.ts`, their rows in `src/lib/page-states.ts`, one test file per page,
`work-claims/task-295-contact-services.md`. NOT `puck-config.tsx`/`copilot.ts`
(flag-and-stop), NOT `src/components/**`, NOT home, NOT wave B pages, NOT the other
pair's pages (/jewelry, /artist).
...
## Report
`~/dev/kimi/outbox/task-295/contact-services/SUMMARY.md` written AS YOU GO, ending once with
`LANE-DONE <full sha>`.

— Ms. Kimi, 0018.06.25 a₿ (block 967,179 at cut)
